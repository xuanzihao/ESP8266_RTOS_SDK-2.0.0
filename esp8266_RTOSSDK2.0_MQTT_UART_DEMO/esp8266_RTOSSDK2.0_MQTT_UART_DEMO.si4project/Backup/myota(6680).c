...
static void hilink_ota_start(void* param)//开始远程升级
{
	...
	totallength=0;
	while ((system_upgrade_flag_check() != UPGRADE_FLAG_FINISH) &&((recbytes = read(ota_socket, recv_buf, 1199)) > 0)) {
...
void ota_start()
{
	...
	strcpy(ota_host,"file2.hxjiot.com");
	strcpy(filename,"/firmware/C85F7DD0B2B000012E82246C1D3C8DD0.bin");
	port=80;
	printf("\r\nparseurlok\r\n");
	if (!ota_task_handle) {
