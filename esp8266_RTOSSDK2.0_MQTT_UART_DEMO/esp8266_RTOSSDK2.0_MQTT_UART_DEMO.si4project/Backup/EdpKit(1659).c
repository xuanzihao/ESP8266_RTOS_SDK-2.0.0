...
static void ICACHE_FLASH_ATTR FormatAt(char* buffer, int len, time_t now)
{
    struct tm timinfo;
...
int32 ICACHE_FLASH_ATTR ReadFloat(EdpPacket* pkg, float* val)
{
    *val = 0;
    if (pkg->_read_pos + 4 > pkg->_write_pos)
        return -1;
...
int32 ICACHE_FLASH_ATTR ReadDataTime(EdpPacket* pkg, LPDataTime val){
    char buf[6] = {0};
    uint8 data_time = 0;
...
//	printf("WriteByte1");
    ...
    if (CheckCapacity(buf, 1))
        return -1;
  //  printf("WriteByte2");
   // printf("buf->_write_pos=%d byte=%d",buf->_write_pos,byte);
...
int32 ICACHE_FLASH_ATTR WriteFloat(Buffer* buf, float val)
{
    (buf->_read_pos == 0);
...
int32 ICACHE_FLASH_ATTR WriteStr(Buffer* buf, const char *str)
{
    uint16 length = 0;
    (buf->_read_pos == 0);
    length = strlen(str);
...
/* sava_data (C->S) */
EdpPacket* ICACHE_FLASH_ATTR PacketSavedataJson(const char* dst_devid, cJSON* json_obj, int type, uint16 msg_id)
{
    EdpPacket* pkg = NULL;
    uint32 remainlen = 0;
    char* json_out = NULL;
...
/* sava_data bin (C->S) */
EdpPacket* ICACHE_FLASH_ATTR PacketSavedataBin(const char* dst_devid, cJSON* desc_obj,
...
EdpPacket* ICACHE_FLASH_ATTR PacketSavedataBinStr(const char* dst_devid, const char* desc_str,
                                const uint8* bin_data, uint32 bin_len, uint16 msg_id)
{
    EdpPacket* pkg = NULL;
    uint32 remainlen = 0;
...
EdpPacket* ICACHE_FLASH_ATTR PacketCmdResp(const char* cmdid, uint16 cmdid_len,
...
int32 ICACHE_FLASH_ATTR IsPkgComplete(RecvBuffer* buf)
{
    uint8* data = NULL;
    uint32 data_len = 0;
    uint32 multiplier = 1;
...
int32 ICACHE_FLASH_ATTR UnpackSavedataJson(EdpPacket* pkg, cJSON** json_obj)
{
    char* json_str;
...
int32 ICACHE_FLASH_ATTR UnpackSavedataBin(EdpPacket* pkg, cJSON** desc_obj, 
...
int32 ICACHE_FLASH_ATTR UnpackSavedataBinStr(EdpPacket* pkg, char** desc_str, 
        uint8** bin_data, uint32* bin_len)
{
    if (ReadStr(pkg, desc_str))
        return ERR_UNPACK_SAVED_BIN_DESC;
...
int32 ICACHE_FLASH_ATTR UnpackCmdReq(EdpPacket* pkg, char** cmdid, uint16* cmdid_len, 
...
/* sava_data (C->S) */
EdpPacket* ICACHE_FLASH_ATTR ICACHE_FLASH_ATTR PacketSavedataSimpleString(const char* dst_devid, const char* input, uint16 msg_id)
{
    EdpPacket* pkg = NULL;
    uint32 remainlen = 0;
    uint32 input_len = 0;
...
int32 ICACHE_FLASH_ATTR UnpackSavedataSimpleString(EdpPacket* pkg, char** output)
{
...
EdpPacket* ICACHE_FLASH_ATTR PacketSavedataSimpleStringWithTime(const char* dst_devid, const char* input, const LPDataTime op_time, uint16 msg_id){
	EdpPacket* pkg = NULL;
    uint32 remainlen = 0;
    uint32 input_len = 0;
    uint16 flag = 0;
...
int32 ICACHE_FLASH_ATTR UnpackSavedataSimpleStringWithTime(EdpPacket* pkg, char** output, LPDataTime at){
...
EdpPacket* ICACHE_FLASH_ATTR PackSavedataFloatWithTime(const char* dst_devid, const FloatDPS* input, int input_count, const LPDataTime op_time, uint16 msg_id){
    EdpPacket* pkg = NewBuffer();
...
int32 ICACHE_FLASH_ATTR UnpackSavedataFloatWithTime(EdpPacket* pkg, FloatDPS** output, int* out_count, LPDataTime at){
    uint32 remain_len = 0;
    uint32 i = 0;
    uint16 count = 0;
    if (ReadDataTime(pkg, at)){
...
EdpPacket* ICACHE_FLASH_ATTR PacketUpdateReq(UpdateInfoList* head)
{
    EdpPacket* pkg = NULL;
    uint32 len = 0;
...
void ICACHE_FLASH_ATTR FreeUpdateInfolist(UpdateInfoList* head)
{
...
int ICACHE_FLASH_ATTR UnpackUpdateResp(EdpPacket* pkg, UpdateInfoList** head)
{
    uint32 remainlen = 0;
    uint32 cur_pos = 0;
    const char* p = NULL;
