...
static int ICACHE_FLASH_ATTR new_tcp_connect(in_addr_t srcip,const char* dst, unsigned short port)
{
	...
	*/
	...
	src.sin_addr.s_addr = srcip;// π”√staµƒ ip
...
void ICACHE_FLASH_ATTR tcp_client_parse(char *buffer ,int len)
{
	...
	DeleteBuffer(&recv_buf);//…æµÙ’‚∏ˆbuf
	DeleteBuffer(&pkg);
}

struct station_config s_staconf;//Ë∑ØÁî±Âô®‰ø°ÊÅØÁªìÊûÑ‰Ωì
uint8 phone_ip[4] = {0,0,0,0};//Áî®Êù•‰øùÂ≠òÊâãÊú∫IPÔºåÂú®mDNS‰∏≠ÁªôÊâãÊú∫ËøõË°åÈâ¥Âà´Ôºõ
void ICACHE_FLASH_ATTR smartconfig_done(sc_status status, void *pdata)
{
    switch(status) {
        ...
        case SC_STATUS_LINK:
		os_printf("\r\nSC_STATUS_LINK\n");
            struct station_config *sta_conf = pdata;//ËÆæÁΩÆWiFi station ÁöÑÂèÇÊï∞ÔºåÂπ∂‰øùÂ≠òÂà∞ flash
               //s_staconf = pdata;
					...
					*/
					switch(tcp_client_state)
					{
						...
						case tcp_client_state_SENDDATA:
						{
							...
							send_pkg = PacketSavedataSimpleString("", (char *)&tcp_send_buffer, msg_id++);//TYPE=5 ,;feild0;feild1;‚Ä¶;feildn
					...
					vTaskDelay(30/portTICK_RATE_MS);//Âª∂Êó∂Êó∂Èó¥‰∏∫30ms
				}
TCP_CLIENT_END:
				new_tcp_disconnect(tcp_fd);
